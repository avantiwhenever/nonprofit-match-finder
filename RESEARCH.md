...
| ProPublica Nonprofit Explorer API | Org mission text, NTEE cause codes, financials, from IRS Form 990 filings, ~1.8M orgs | Free, keyless | Good for enrichment/cause-area context, not opportunity listings. https://projects.propublica.org/nonprofits/api |
| Data.gov volunteer datasets | Static local datasets (e.g. NYC's Volunteer Opportunities dataset) | Free, no key | Useful for prototyping without an approval wait; check for WA-specific equivalents |
| IRS Exempt Organizations Business Master File | All registered nonprofits by state/address/NTEE code | Free, public | Backup/cross-reference source for org identity data |
